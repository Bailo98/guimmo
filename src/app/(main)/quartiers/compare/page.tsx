...
function Stars({ value }: { value: number }) {
  return (
    <div className="flex gap-0.5">
      {[1,2,3,4,5].map(i => (
        <div key={i} className={`w-3 h-3 rounded-sm ${i <= value ? "bg-[var(--accent-gold)]" : "bg-slate-200 dark:bg-slate-700"}`} />
      ))}
    </div>
  );
}
  ...
  return (
    <div className="max-w-5xl mx-auto px-4 py-8 pb-24">
      <nav className="flex items-center gap-2 text-xs text-slate-400 mb-6">
        ...
        <span>/</span>
        <Link href="/quartiers/kipe" className="hover:text-[var(--accent-gold)]">Quartiers</Link>
        <span>/</span>
          ...
          <button
            ...
            className={`flex items-center gap-1.5 px-3 py-1.5 rounded-full text-sm font-semibold border transition-all ${
              selected.includes(n.id)
                ...
                : "border-slate-200 dark:border-[var(--border)] text-slate-600 dark:text-slate-300 hover:border-[var(--accent-gold)] hover:text-[var(--accent-gold)]"
            }`}
          >
      ...
      <div className="mt-8 grid grid-cols-1 md:grid-cols-3 gap-4">
        {neighborhoods.map(n => (
          <Link
            key={n.id}
            href={`/quartiers/${n.id}`}
            className="flex items-center justify-between p-4 bg-[var(--bg-card-light)] rounded-2xl border border-[var(--border)] hover:border-[var(--accent-gold)] transition-colors group"
          >
            <div>
            ...
            <MapPin className="w-5 h-5 text-[var(--accent-gold)] group-hover:scale-110 transition-transform" />
          </Link>
        ))}
      </div>
    </div>
