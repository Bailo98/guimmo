...
function ConnexionForm() {
  ...
  // Register the global callback GIS calls after sign-in
  useEffect(() => {
    ...
    (window as any).__guimmoGoogleCallback = async (response: { credential: string }) => {
      ...
      if (signInError) {
        setError(`Erreur Google: ${signInError.message}`);
        setLoading(false);
      } else {
    ...
    return () => {
      // eslint-disable-next-line @typescript-eslint/no-explicit-any
      delete (window as any).__guimmoGoogleCallback;
    };
  }, [redirect, router]);
  ...
  return (
    <div className="min-h-screen bg-[#111418] flex flex-col">
      {/* GIS HTML API: auto-initializes when script loads */}
      {GOOGLE_CLIENT_ID && (
        <div
          id="g_id_onload"
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-8">
        <div className="w-full max-w-sm">
          <div className="bg-[#1e2430] rounded-3xl p-8 border border-[#2a3040]">
            ...
            {/* Google button — always visible, GIS handles auth on click */}
            <button
              type="button"
              disabled={loading}
              onClick={() => {
                ...
                if (gis) { gis.prompt(); } else { setError("Google Sign-In n'est pas disponible. Rechargez la page."); }
              }}
              className="flex items-center justify-center gap-3 w-full bg-white hover:bg-slate-50 disabled:opacity-50 text-slate-700 font-semibold py-3 rounded-xl border border-slate-200 transition-colors text-sm mb-3"
            >
