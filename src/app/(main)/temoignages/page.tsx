  ...
  return (
    <div className="flex items-center gap-0.5">
      {Array.from({ length: 5 }).map((_, i) => (
        <Star
          key={i}
          className={`w-4 h-4 ${i < rating ? "fill-[#F97316] text-[#F97316]" : "text-slate-300 dark:text-slate-600"}`}
        />
      ))}
    </div>
  );
      ...
      <div className="bg-white dark:bg-[#1e2430] border-t border-slate-100 dark:border-[#2a3040]">
        <div className="max-w-2xl mx-auto px-4 py-16 text-center">
          ...
          <div className="flex flex-col sm:flex-row gap-3 justify-center">
            <Link
              href="/annonces"
              className="inline-flex items-center justify-center gap-2 bg-[#F97316] hover:bg-[#ea6c0a] text-white font-bold px-7 py-3.5 rounded-xl transition-colors"
            >
              Chercher un logement
            </Link>
            <Link
              href="/inscription"
              className="inline-flex items-center justify-center gap-2 bg-white dark:bg-[#2a3040] border border-slate-200 dark:border-slate-600 text-slate-800 dark:text-white font-bold px-7 py-3.5 rounded-xl hover:border-[#F97316] transition-colors"
            >
              Créer un compte
            </Link>
          </div>
...
function TestimonialCard({
  ...
  return (
    <div className="bg-white dark:bg-[#1e2430] rounded-2xl border border-slate-100 dark:border-[#2a3040] p-5 flex flex-col gap-3">
      ...
      {/* Saved badge */}
      {testimonial.saved && (
        <div className="inline-flex items-center gap-1.5 bg-green-50 dark:bg-green-900/20 text-green-700 dark:text-green-400 text-xs font-semibold px-3 py-1.5 rounded-full w-fit">
      ...
      <div className="flex items-center gap-3 pt-1 border-t border-slate-100 dark:border-[#2a3040] mt-auto">
        <div className="w-10 h-10 rounded-full bg-gradient-to-br from-[#F97316] to-[#ea6c0a] flex items-center justify-center text-white font-bold text-sm flex-shrink-0">
