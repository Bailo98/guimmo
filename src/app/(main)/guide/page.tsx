...
export default function GuidePage() {
  ...
  return (
    <div className="max-w-4xl mx-auto px-4 py-10">
      ...
      {/* Hero */}
      <div className="mb-12">
        <div className="inline-flex items-center gap-2 bg-[var(--accent-gold)]/10 text-[var(--accent-gold)] text-xs font-bold px-3 py-1.5 rounded-full mb-4">
          Guide complet
        </div>
      ...
      <div className="space-y-10">
        {SECTIONS.map((section, sIdx) => {
          ...
          return (
            <section
              key={section.id}
              id={section.id}
              className={`rounded-3xl p-6 md:p-8 ${sIdx % 2 === 0 ? "bg-[var(--bg-card-light)] border border-[var(--border)]" : "bg-[var(--bg-card-light)] border border-[var(--border)]"}`}
            >
        ...
        <section id="quartiers" className="bg-[var(--bg-card-light)] rounded-3xl p-6 md:p-8 border border-[var(--border)]">
          <div className="flex items-center gap-3 mb-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
            {popularNeighborhoods.map((n) => (
              <Link
                ...
                className="group flex flex-col items-center gap-2 p-3 rounded-2xl bg-[var(--bg-card-light)] border border-[var(--border)] hover:border-[var(--accent-gold)] hover:bg-[#B8963A]/5 transition-all text-center"
              >
                <MapPin className="w-5 h-5 text-slate-300 dark:text-slate-600 group-hover:text-[var(--accent-gold)] transition-colors" />
                <span className="text-xs font-semibold text-slate-700 dark:text-slate-300 group-hover:text-[var(--accent-gold)] transition-colors">{n.name}</span>
      ...
      <div className="mt-12 bg-[#111418] dark:bg-[#0d1014] rounded-3xl p-8 text-center">
        ...
        <Link
          href="/annonces"
          className="inline-flex items-center gap-2 bg-[var(--accent-gold)] text-white font-bold px-8 py-3.5 rounded-2xl hover:bg-[#B8963A] transition-colors"
        >
