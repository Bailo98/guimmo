...
const INITIAL_CONTACT = {
  email: "contact@LogerBien.gn",
  emailSupport: "support@LogerBien.gn",
  phone: "+224 628 222 510",
  whatsapp: "+224 628 222 510",
  facebook: "https://facebook.com/LogerBien",
  instagram: "https://instagram.com/LogerBien",
  youtube: "",
  address: "Kipé, Conakry, Guinée",
  website: "https://LogerBien-orcin.vercel.app",
};

const INITIAL_ABOUT = {
  title: "La plateforme immobilière de confiance en Guinée",
  subtitle: "LogerBien connecte propriétaires et locataires en toute sécurité.",
...
export default function AdminContenuPage() {
  ...
  useEffect(() => {
    try {
      const c = localStorage.getItem("LogerBien-contact");
      if (c) setContact((prev) => ({ ...prev, ...JSON.parse(c) }));
      const a = localStorage.getItem("LogerBien-about");
      if (a) setAbout((prev) => ({ ...prev, ...JSON.parse(a) }));
    } catch { /* ignore */ }
  }, []);

  function handleSave() {
    localStorage.setItem("LogerBien-contact", JSON.stringify(contact));
    localStorage.setItem("LogerBien-about", JSON.stringify(about));
    setSaved(true);
  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        <button
          onClick={handleSave}
          className="flex items-center gap-2 bg-[var(--accent-gold)] text-white font-bold px-4 py-2.5 rounded-xl hover:bg-[#B8963A] transition-colors"
        >
      ...
      {/* Tabs */}
      <div className="flex gap-2 border-b border-slate-200 dark:border-[var(--border)]">
        {TABS.map((t) => {
          const Icon = t.icon;
          return (
            <button
      ...
      {tab === "contact" && (
        <div className="bg-[var(--bg-card-light)] rounded-2xl p-6 border border-[var(--border)] space-y-5">
          <h2 className="font-bold text-slate-900 dark:text-white">Coordonnées de LogerBien</h2>

          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { key: "email", label: "Email principal", icon: Mail, placeholder: "contact@LogerBien.gn" },
              { key: "emailSupport", label: "Email support", icon: Mail, placeholder: "support@LogerBien.gn" },
              ...
              { key: "facebook", label: "Page Facebook", icon: Link, placeholder: "https://facebook.com/LogerBien" },
              { key: "instagram", label: "Instagram", icon: Link, placeholder: "https://instagram.com/LogerBien" },
              { key: "youtube", label: "YouTube", icon: Video, placeholder: "https://youtube.com/@LogerBien" },
              { key: "website", label: "Site web", icon: Globe, placeholder: "https://LogerBien.gn" },
            ].map((f) => {
              const Icon = f.icon;
              return (
                <div key={f.key}>
                  ...
                  <div className="relative">
                    ...
                    <input
                      ...
                      className="w-full bg-[var(--bg-card-light)] border border-[var(--border)] rounded-xl pl-9 pr-4 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--accent-gold)]"
                    />
                  </div>
                </div>
              );
          ...
          <div>
            ...
            <input
              ...
              className="w-full bg-[var(--bg-card-light)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--accent-gold)]"
            />
          </div>
        </div>
      )}
            ...
            {[
              ...
              <div key={f.key}>
                ...
                <textarea
                  ...
                  value={about[f.key as keyof typeof about]}
                  onChange={(e) => setAbout({ ...about, [f.key]: e.target.value })}
                  className="w-full bg-[var(--bg-card-light)] border border-[var(--border)] rounded-xl px-4 py-2.5 text-sm text-slate-900 dark:text-white focus:outline-none focus:ring-2 focus:ring-[var(--accent-gold)] resize-none"
                />
              </div>
            ))}
...
function TeamEditor() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("LogerBien-team");
  ...
  function save(next: typeof members) {
    setMembers(next);
    try { localStorage.setItem("LogerBien-team", JSON.stringify(next)); } catch { /* ignore */ }
  }
  ...
  return (
    <div className="space-y-4">
      {members.map((m) => (
        <div key={m.id} className="bg-[var(--bg-card-light)] rounded-2xl p-5 border border-[var(--border)]">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            {[
              { key: "name", label: "Nom" },
              ...
              <div key={f.key}>
                ...
                <input
                  ...
                  className="w-full bg-[var(--bg-card-light)] border border-[var(--border)] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--accent-gold)] text-slate-900 dark:text-white"
                />
              </div>
            ))}
            <div>
              ...
              <textarea
                ...
                className="w-full bg-[var(--bg-card-light)] border border-[var(--border)] rounded-xl px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-[var(--accent-gold)] resize-none text-slate-900 dark:text-white"
              />
            </div>
          </div>
      ...
      <button
        onClick={addMember}
        className="w-full py-3 rounded-2xl border-2 border-dashed border-slate-200 dark:border-[var(--border)] text-slate-500 hover:border-[var(--accent-gold)] hover:text-[var(--accent-gold)] transition-colors text-sm font-semibold"
      >
        + Ajouter un membre
      </button>
    </div>
...
function BlogEditor() {
  ...
  useEffect(() => {
    try {
      const saved = localStorage.getItem("LogerBien-blog-posts");
  ...
  function savePosts(next: typeof posts) {
    setPosts(next);
    try { localStorage.setItem("LogerBien-blog-posts", JSON.stringify(next)); } catch { /* ignore */ }
  }
  ...
  return (
    <div className="space-y-4">
      <button
        onClick={addPost}
        className="flex items-center gap-2 bg-[var(--accent-gold)] text-white font-bold px-4 py-2.5 rounded-xl hover:bg-[#B8963A] transition-colors text-sm"
      >
        + Nouvel article
      </button>

      {posts.map((post) => (
        <div key={post.id} className="bg-[var(--bg-card-light)] rounded-2xl border border-[var(--border)] overflow-hidden">
          <div className="p-4 flex items-center justify-between">
            <div>
              <p className="font-semibold text-slate-900 dark:text-white text-sm">{post.title || "Nouvel article"}</p>
              <div className="flex items-center gap-3 mt-1">
                <span className="text-xs text-slate-400">{post.date}</span>
                <span className={`text-xs font-semibold ${post.published ? "text-green-500" : "text-[var(--accent-gold)]"}`}>
          ...
          {editing === post.id && (
            <div className="border-t border-slate-100 dark:border-[var(--border)] p-4 space-y-3">
