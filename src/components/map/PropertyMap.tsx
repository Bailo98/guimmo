...
function buildMarkers(
    ...
    if (group.length === 1) {
      ...
      const popup = L.popup({ maxWidth: 240, className: "leaflet-popup-custom" }).setContent(`
        <div style="font-family:system-ui,sans-serif;padding:4px 0;">
          ...
          <p style="font-size:12px;color:var(--accent-gold);font-weight:600;margin:0 0 8px;">${formattedPrice}${property.price_period === "month" ? "/mois" : ""}</p>
          <a href="/annonces/${property.id}" style="display:inline-block;background:var(--accent-gold);color:#fff;font-size:12px;font-weight:600;padding:5px 12px;border-radius:8px;text-decoration:none;">Voir →</a>
        </div>
      `);
      ...
      const clusterIcon = L.divIcon({
        html: `<div style="background:var(--accent-gold);color:white;width:36px;height:36px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-weight:bold;font-size:14px;border:3px solid white;box-shadow:0 2px 8px rgba(0,0,0,0.3)">${count}</div>`,
        iconSize: [36, 36],
        className: "",
      });
      ...
      const listItems = group
        .map(
          (p) =>
            `<li style="margin:0 0 6px;"><a href="/annonces/${p.id}" style="color:var(--accent-gold);font-weight:600;font-size:13px;text-decoration:none;">${p.title}</a></li>`
        )
        .join("");
  ...
  if (showHeatmap && properties.length > 0) {
    ...
    Object.entries(neighborhoodPrices).forEach(([neighborhood, prices]) => {
      ...
      const color = ratio < 0.33 ? "var(--accent-gold)" : ratio < 0.66 ? "var(--accent-gold)" : "#ef4444";
        ...
        <button
          ...
          className={cn(
            ...
            heatmap
              ...
              : "bg-[var(--bg-card-light)] text-slate-700 dark:text-white border-slate-200 dark:border-[var(--border)]"
          )}
        >
