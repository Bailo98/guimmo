            ...
            <div className="flex flex-col gap-3">
              {[
                ...
                <button
                  key={label}
                  onClick={() => setStep(1)}
                  className="flex items-center gap-3 w-full px-5 py-4 rounded-xl border-2 border-[var(--border)] bg-[var(--bg-card-light)] text-white font-semibold text-sm hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors text-left"
                >
            ...
            <div className="grid grid-cols-2 gap-3">
              {BUDGET_OPTIONS.map((option) => (
                <button
                  key={option}
                  onClick={() => setStep(2)}
                  className="px-4 py-4 rounded-xl border-2 border-[var(--border)] bg-[var(--bg-card-light)] text-white font-semibold text-sm hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors text-center"
                >
                  {option}
                </button>
              ))}
            ...
            <div className="grid grid-cols-3 gap-3">
              {NEIGHBORHOODS.map((name) => (
                <button
                  key={name}
                  onClick={() => finish()}
                  className="px-3 py-3 rounded-xl border-2 border-[var(--border)] bg-[var(--bg-card-light)] text-white font-semibold text-sm hover:border-[#D4AF37] hover:text-[#D4AF37] transition-colors text-center"
                >
                  {name}
                </button>
              ))}
