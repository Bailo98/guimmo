...
export function NotificationBell() {
  ...
  function getIcon(type: "success" | "info" | "warning") {
    ...
    if (type === "warning") return <AlertTriangle className="w-4 h-4 text-[var(--accent-gold)] shrink-0" />;
  ...
  return (
    <div ref={containerRef} className="relative">
      ...
      >
        ...
        {count > 0 && (
          <span className="absolute -top-0.5 -right-0.5 min-w-[18px] h-[18px] rounded-full bg-[var(--accent-gold)] text-white text-[10px] font-bold flex items-center justify-center px-0.5 leading-none">
            {count > 9 ? "9+" : count}
          </span>
        )}
      </button>
      ...
      {open && (
        <div className="absolute right-0 top-11 w-80 rounded-2xl shadow-2xl border border-[var(--border)] bg-[var(--bg-card-light)] z-50 overflow-hidden">
          <div className="flex items-center justify-between px-4 py-3 border-b border-[var(--border)]">
            <span className="font-semibold text-sm text-slate-800 dark:text-slate-100">
              Notifications {count > 0 && <span className="text-[var(--accent-gold)]">({count})</span>}
            </span>
            {count > 0 ? (
              <button
                onClick={handleMarkAllRead}
                className="flex items-center gap-1 text-xs font-medium text-[var(--accent-gold)] hover:underline"
              >
              ...
              {notifications.slice(0, 8).map((n) => (
                <li
                  ...
                  className={cn(
                    ...
                    !n.read ? "bg-orange-50 dark:bg-orange-900/10" : "bg-[var(--bg-card-light)]"
                  )}
                >
                  {getIcon(n.type)}
                  ...
                  {!n.read && <div className="w-2 h-2 bg-[var(--accent-gold)] rounded-full flex-shrink-0 mt-1.5" />}
                </li>
              ))}
            </ul>
          )}
