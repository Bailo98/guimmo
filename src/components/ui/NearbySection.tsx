...
export function NearbySection({ properties, horizontal = false }: Props) {
  ...
  const header = (
    <div className="flex items-center gap-2 mb-4">
      <div className="w-7 h-7 bg-[var(--accent-gold)]/10 rounded-lg flex items-center justify-center">
        ...
        <div className="flex gap-4 overflow-x-auto pb-2 scrollbar-hide snap-x snap-mandatory">
          {nearby.map((p, i) => (
            <div key={p.id} className="relative flex-none w-64 snap-start">
              ...
              <div className="absolute top-3 left-3 bg-[var(--accent-gold)] text-white text-[10px] font-bold px-2 py-0.5 rounded-full pointer-events-none">
                {p.distanceKm < 1
  ...
  return (
    <section className="mb-10">
      ...
      <div className="mt-4 border-t border-slate-100 dark:border-[var(--border)]" />
    </section>
  );
}
