...
function scoreColor(score: number): string {
  if (score >= 71) return "#6ec97a";
  if (score >= 41) return "#daa84a";
  return "#f87171";
}
  ...
  if (props.compact) {
    return (
      <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
        <div style={{ flex: 1, height: 4, background: "rgba(240,230,204,0.10)", borderRadius: 999, overflow: "hidden" }}>
  ...
  return (
    <div style={{
      ...
      <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginBottom: 10 }}>
        <p style={{ color: "rgba(240,230,204,0.55)", fontSize: 11, fontWeight: 700, letterSpacing: "1px", textTransform: "uppercase", margin: 0 }}>
          Score de confiance
        </p>
        <div style={{ display: "flex", alignItems: "baseline", gap: 4 }}>
          ...
          <span style={{ fontSize: 12, color: "rgba(240,230,204,0.45)" }}>/100</span>
          <span style={{ fontSize: 11, fontWeight: 700, color, marginLeft: 4 }}>— {scoreLabel(score)}</span>
        </div>
      </div>

      {/* Bar */}
      <div style={{ height: 8, background: "rgba(240,230,204,0.08)", borderRadius: 999, overflow: "hidden", marginBottom: 12 }}>
        ...
        {[
          ...
          .map((c) => (
            <div key={c.label} style={{ display: "flex", alignItems: "center", gap: 5 }}>
              <span style={{ fontSize: 11, color: c.ok ? "#6ec97a" : "rgba(240,230,204,0.30)" }}>
                {c.ok ? "✓" : "○"}
              </span>
              <span style={{ fontSize: 11, color: c.ok ? "rgba(240,230,204,0.70)" : "rgba(240,230,204,0.30)" }}>
                {c.label}
              </span>
              <span style={{ fontSize: 10, color: c.ok ? color : "rgba(240,230,204,0.20)", fontWeight: 700 }}>
                {c.pts}
              </span>
            </div>
          ))}
